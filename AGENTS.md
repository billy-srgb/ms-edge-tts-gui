...
- 版本流程：改代码 → bump `src/edgettsgui/config.py` 的 APP_VERSION 与 installer/EdgeTTSGui.iss → RELEASE_NOTES 更新 → 本地 `make windows` / `make macos` / `make linux` → commit → push main → tag vX.Y.Z → push tag（CI 自动发布三端安装包）。
